...
function makeIntrinsicsCollector() {
  ...
  const intrinsicsCollector = {
    ...
    finalIntrinsics() {
      freeze(intrinsics);
      pseudoNatives = new WeakSet(values(intrinsics).filter((obj) => typeof obj === 'function'));
      return intrinsics;
    },
    isPseudoNative(obj) {
      if (!pseudoNatives) {
        throw new Error('isPseudoNative can only be called after finalIntrinsics');
      }
  ...
  intrinsicsCollector.addIntrinsics(sampleGlobals(globalThis, universalPropertyNames));
...
function getGlobalIntrinsics(globalObject) {
  ...
  intrinsicsCollector.addIntrinsics(sampleGlobals(globalObject, sharedGlobalPropertyNames));
 ...
 */
const makeError = (optDetails = redactedDetails`Assert failed`, ErrorConstructor = Error) => {
...
const makeAssert = (optRaise = undefined, unredacted = false) => {
  ...
  /** @type {AssertFail} */
  const fail = (optDetails = details`Assert failed`, ErrorConstructor = Error) => {
  ...
  /** @type {BaseAssert} */
  function baseAssert(flag, optDetails = details`Check failed`, ErrorConstructor = Error) {
    if (!flag) {
  ...
  /** @type {AssertString} */
  const assertString = (specimen, optDetails) => assertTypeof(specimen, 'string', optDetails);
...
function getScopeConstants(globalObject, localObject = {}) {
  ...
  const localConstants = localNames.filter(
    (name) => isValidIdentifierName(name) && isImmutableDataProperty(localObject, name)
  );
  ...
  const globalConstants = globalNames.filter(
    (name) =>
      ...
      !localNames.includes(name) && isValidIdentifierName(name) && isImmutableDataProperty(globalObject, name)
  );
...
const alwaysThrowHandler = new Proxy(immutableObject, {
  get(_shadow, prop) {
    assert.fail(d$1`Please report unexpected scope handler trap: ${q$1(String(prop))}`);
  },
});
 ...
 */
function createScopeHandler(globalObject, localObject = {}, { sloppyGlobalsMode = false } = {}) {
  return {
    ...
    has(_shadow, prop) {
      ...
      if (sloppyGlobalsMode || prop === 'eval' || prop in localObject || prop in globalObject || prop in globalThis) {
        return true;
      }
    ...
    getOwnPropertyDescriptor(_target, prop) {
      ...
      console.warn(`getOwnPropertyDescriptor trap on scopeHandler for ${quotedProp}`, new Error().stack);
      return undefined;
    },
  };
}
...
function performEval(
  ...
  globalObject,
  localObject = {},
  { localTransforms = [], globalTransforms = [], sloppyGlobalsMode = false, knownScopeProxies = new WeakSet() } = {}
) {
...
function makeFunctionConstructor(globaObject, options = {}) {
  ...
  assert(getPrototypeOf(Function) === Function.prototype, 'Function prototype is the same accross compartments');
  assert(
...
function initGlobalObject(
  ...
  perCompartmentGlobals.Compartment = makeCompartmentConstructor(makeCompartmentConstructor, intrinsics, nativeBrander);
...
const makeCausalConsole = (baseConsole, loggedErrorHandler) => {
  const { getStackString, takeMessageLogArgs, takeNoteLogArgsArray } = loggedErrorHandler;
  ...
  const logError = (error) => {
    ...
    if (typeof stackString === 'string' && stackString.length >= 1 && !stackString.endsWith('\n')) {
      stackString += '\n';
    }
  ...
  const otherMethodNames = consoleOtherMethods.filter(([name, _]) => name in baseConsole);
 ...
 */
const tameConsole = (consoleTaming = 'safe', optGetStackString = undefined) => {
...
function tameV8ErrorConstructor(OriginalError, InitialError, errorTaming, stackFiltering) {
  ...
  const stackStringFromSST = (_error, sst) => [...sst.filter(callSiteFilter).map(callSiteStringifier)].join('');
...
function tameErrorConstructor(errorTaming = 'safe', stackFiltering = 'concise') {
  ...
  const platform = typeof OriginalError.captureStackTrace === 'function' ? 'v8' : 'unknown';
  ...
  if (platform === 'v8') {
    initialGetStackString = tameV8ErrorConstructor(OriginalError, InitialError, errorTaming, stackFiltering);
  }
  return {
 ...
 */
function repairIntrinsics(makeCompartmentConstructor, compartmentPrototype, getAnonymousIntrinsics, options = {}) {
  ...
  assert(extraOptionsNames.length === 0, d$3`lockdown(): non supported option ${q$2(extraOptionsNames)}`);
  ...
  intrinsicsCollector.addIntrinsics(tameErrorConstructor(errorTaming, stackFiltering));
 ...
 * @param {() => Object} getAnonymousIntrinsics
 */
const makeLockdown = (makeCompartmentConstructor, compartmentPrototype, getAnonymousIntrinsics) => {
  /**
   * @param {LockdownOptions} [options]
   */
...
const makeCompartmentConstructor = (targetMakeCompartmentConstructor, intrinsics, nativeBrander) => {
  ...
  function Compartment(endowments = {}, _moduleMap = {}, options = {}) {
    ...
    // Extract options, and shallow-clone transforms.
    const { name = '<unknown>', transforms = [], __shimTransforms__ = [], globalLexicals = {} } = options;
...
assign(globalThis, {
  harden,
  lockdown: makeLockdown(makeCompartmentConstructor, CompartmentPrototype, getAnonymousIntrinsics),
  Compartment,
  assert,
});
